    ...
    <div>
      ...
      >
        <van-cell style="font-size: 12px" v-for="item in appointList" :key="item.id">
          ...
          <van-row>
            <van-col style="text-align: center;color: #3b86e8" span="24">
              <van-button @click="confirmYarn(item)" round color="linear-gradient(to right, #ff6034, #ee0a24)" block>
                选择该客户
              </van-button>
            </van-col>
          </van-row>
...
export default {
  ...
  methods: {
    ...
    confirmYarn: function (data) {
      var clothesAllName = this.clothes.styleType+'-'+this.clothes.styleName+'-'+this.clothes.clothesSize+'-'+this.clothes.clothesNo;
      this.$dialog.confirm({
        message: "是否给"+"<a style='color: #39a9ed'>"
            +data.name+"<a>"+"添加"+"" +
            "<a style='color: #39a9ed'>"+clothesAllName+"<a>"+"的试纱？",
        theme: 'round-button',
      }).then(() => {
        this.addClothesYarn(data);
      },() =>{})
    },
    addClothesYarn: function (data) {
