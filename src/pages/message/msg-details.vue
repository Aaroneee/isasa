        ...
        <van-row gutter="20">
          ...
          <van-col :span="13">
            ...
            <van-button v-if="item.isShowState===0"
                        ...
                        @click="toStoreStyleAdd(item)"
                        text="添加至款式"/>
            <van-row v-else>
              <p style="font-size: 16px;font-weight: bolder;color: var(--my-success-color);margin-top: 2%">已添加</p>
            </van-row>
          </van-col>
        </van-row>
      </div>
...
export default {
  ...
  methods: {
    ...
    toStoreStyleAdd(item) {
      let queryObj = {
        msgId: this.$route.query.id,
        storeStyleId: item.storeStyleId,
        clothesSize: item.clothesSize
      };
      this.$router.push({name: 'storeStyleAdd', query: queryObj})
    }
  },
}
</script>
